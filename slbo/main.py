...
def make_real_runner(n_envs,goal):
    from slbo.envs.batched_env import BatchedEnv
    batched_env = BatchedEnv([make_env(FLAGS.env.id,goal) for _ in range(n_envs)])
    return Runner(batched_env, rescale_action=True, **FLAGS.runner.as_dict())

def virtual_training(FLAGS,model,policy,vfn,algo,train_set,loss_mod,settings,runners,max_ent_coef):
...
def main():
    ...
    # For swimmer
    task_goals = np.linspace(0.1,2.0,num_task)
    ...
    for task_goal in task_goals:
        ...
        #new random environment for every new task
        env = make_env(FLAGS.env.id,task_goal)
        env.verify()
        ...
        runners = {
            'test': make_real_runner(4,task_goal),
            'collect': make_real_runner(1,task_goal),
            'dev': make_real_runner(1,task_goal),
            'train': make_real_runner(FLAGS.plan.n_envs,task_goal) if FLAGS.algorithm == 'MF' else virt_runner,
        }
        ...
        for T in range(FLAGS.slbo.n_stages):
            ...
            add_multi_step(runners['dev'].run(noise.make(policy), FLAGS.rollout.n_dev_samples)[0],
            dev_set)

            returns = np.array([ep_info['return'] for ep_info in ep_infos])
            if len(returns) > 0:
                logger.info("episode: %s", np.mean(returns))
            task_returns.append(np.mean(returns))
            ...
            if T !=0:
                #warm-up
                virtual_training(FLAGS,model,policy,vfn,algo,train_set,loss_mod,settings,runners,max_ent_coef)
