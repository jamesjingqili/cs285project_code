...
class RLLabSwimmerEnv(mujoco_env.MujocoEnv, utils.EzPickle, BaseModelBasedEnv):
    ...
    def step(self, action: np.ndarray):
        ...
        ctrl_cost = self.control_coeff * np.sum(np.square(action / scaling))
    ...
    def mb_step(self, states: np.ndarray, actions: np.ndarray, next_states: np.ndarray):
        scaling = 0.5 * (self.action_space.high - self.action_space.low)
        ctrl_cost = self.control_coeff * np.sum(np.square(actions / scaling), axis=-1)
    ...
    def change_ctrl_coeff(self,coeff_scale):
        self.control_coeff = self.control_coeff*coeff_scale