...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, max_episode_steps, test=True,goal=None):
    def _thunk():
        if test:
            env = gym.make(env_id)
        else:
            env = make_mujoco_env(env_id,goal)
...
def make_vec_envs(env_name: str,
    ...
    envs = [
        make_env(env_name, seed, i, log_dir, allow_early_resets, max_episode_steps, test,goal)
...
def make_vec_virtual_envs(env_name: str,
    ...
    envs = VecVirtualEnv(dynamics, make_mujoco_env(env_name,goal), num_envs, seed, max_episode_steps)
