...
def get_args():
    ...
    parser.add_argument('--total-iteration-num', 
                        type=int, default = 10) # total number of iterations, 
    # in each iteration, train each task policy and the distilled policy
    parser.add_argument('--distilled-policy-training-epoch', 
                        type = int, default = 10) # training times in each iteration
    parser.add_argument(
        '--device', type=str,
...
gym.envs.register(
     ...
     kwargs={'state_bound' : 50., 'action_bound':50., 'state_size': 16, 'action_size': 2, 'dynamics_model_path': 'slbo_pytorch/result/Dec12_053104/save/actor_critic_dynamics_epoch9.pt'},
)

args = get_args()
#args.task = 'LearnedSwimmer-v0'
# the belows are for debug
#args.epoch=1
#args.step_per_epoch = 50
#args.total_iteration_num = 2
#args.distilled_policy_training_epoch=2
# the aboves are for debug
...
current_date_time = "{}_{}".format(current_date, current_time)
log_path = os.path.join(args.logdir, args.task, 'distral', current_date_time)
#writer = SummaryWriter(log_path)
print("The log files are stored at : {}".format(log_path))
...
for itr in range(args.total_iteration_num):
    if itr == 0:
        # train policy 1
        print('Iteration {}, training policy_1'.format(itr))
        policy_log_path = os.path.join(log_path, "iter_{}_policy_1").format(itr)
        data_log_path = os.path.join(log_path, "iter_{}_policy_1_data").format(itr)
        result = offpolicy_trainer(
                policy_1, train_collector_1, test_collector_1, args.epoch,
                args.step_per_epoch, args.collect_per_step, args.test_num,
                args.batch_size, stop_fn=stop_fn, 
            save_fn=lambda policy: torch.save(policy.state_dict(), policy_log_path), 
            writer=SummaryWriter(data_log_path))
        ...
        # train policy 2
        print('Iteration {}, training policy_2'.format(itr))
        policy_log_path = os.path.join(log_path, "iter_{}_policy_2").format(itr)
        data_log_path = os.path.join(log_path, "iter_{}_policy_2_data").format(itr)
        result = offpolicy_trainer(
        ...
        print('Iteration {}, training the distilled policy'.format(itr))
        policy_log_path = os.path.join(log_path, "iter_{}_policy_0").format(itr)
        data_log_path = os.path.join(log_path, "iter_{}_policy_0_data").format(itr)
        if args.task == 'Pendulum-v0':
        ...
        result = Distral_offpolicy_trainer(
                    ...
                    distilled_policy_test_collector, args.distilled_policy_training_epoch,
        ...
        if __name__ == '__main__':
            pprint.pprint(result)
            # Let's watch its performance!
            print('test the distilled policy in the first task')
