...
class DistralBasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.learn`: update policy with a given \
    ...
    @abstractmethod
    def learn(
        self, batch1: Batch,batch2: Batch,  **kwargs: Any
    ...
    def update(
        self, sample_size: int, buffer1: Optional[ReplayBuffer], buffer2:Optional[ReplayBuffer],  **kwargs: Any
    ) -> Mapping[str, Union[float, List[float]]]:
        ...
        if buffer1 is None:
            return {}
        batch1, indice1 = buffer1.sample(sample_size)
        batch2, indice2 = buffer2.sample(sample_size)
        self.updating = True
        #import pdb; pdb.set_trace()
        batch1 = self.process_fn(batch1, buffer1, indice1)
        batch2 = self.process_fn(batch2, buffer2, indice2)
        result = self.learn(batch1, batch2,**kwargs)
        self.post_process_fn(batch1, buffer1, indice1)
        self.post_process_fn(batch2, buffer2, indice2)
        self.updating = False
        return result
